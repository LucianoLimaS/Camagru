<!-- Toggle Container (Login / Sign Up) -->
<div class="absolute top-margin-desktop right-margin-desktop flex space-x-sm bg-surface-container-low p-xs rounded-full">
    <button class="px-sm py-xs rounded-full font-label-md text-label-md bg-white shadow-sm text-primary transition-all" id="toggleLoginBtn">Login</button>
    <button class="px-sm py-xs rounded-full font-label-md text-label-md text-secondary hover:text-primary transition-all" id="toggleSignupBtn">Sign Up</button>
</div>
...
<div class="w-full max-w-[400px] mx-auto transition-all duration-500 transform translate-x-0 opacity-100" id="loginFormContainer">
    <div class="mb-lg">
        <h2 class="font-headline-md text-headline-md text-on-background mb-xs">Welcome Back</h2>
        <p class="font-body-sm text-body-sm text-on-surface-variant">Please enter your details to sign in.</p>
    </div>
    <form class="space-y-md" method="POST" action="/login">
        <div class="space-y-xs">
            <label class="block font-label-sm text-label-sm text-on-surface" for="login-username">Username</label>
            <input class="form-input w-full px-sm py-[10px] rounded text-body-md text-on-background bg-surface-container-lowest" id="login-username" name="username" placeholder="Enter your username" type="text" required/>
        </div>
        <div class="space-y-xs">
            <div class="flex justify-between items-center">
                <label class="block font-label-sm text-label-sm text-on-surface" for="login-password">Password</label>
...
<div class="w-full max-w-[400px] mx-auto hidden absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-full px-margin-desktop md:px-lg" id="signupFormContainer">
    <div class="mb-lg">
        <h2 class="font-headline-md text-headline-md text-on-background mb-xs">Create Account</h2>
        <p class="font-body-sm text-body-sm text-on-surface-variant">Join Camagru to start sharing.</p>
    </div>
    <form class="space-y-md" method="POST" action="/register">
        <div class="space-y-xs">
            <label class="block font-label-sm text-label-sm text-on-surface" for="signup-email">Email Address</label>
            <input class="form-input w-full px-sm py-[10px] rounded text-body-md text-on-background bg-surface-container-lowest" id="signup-email" name="email" placeholder="you@example.com" type="email" required/>
        </div>
        <div class="space-y-xs">
            <label class="block font-label-sm text-label-sm text-on-surface" for="signup-username">Username</label>
            <input class="form-input w-full px-sm py-[10px] rounded text-body-md text-on-background bg-surface-container-lowest" id="signup-username" name="username" placeholder="Choose a unique username" type="text" required/>
        </div>
        <div class="space-y-xs">
            <label class="block font-label-sm text-label-sm text-on-surface" for="signup-password">Password</label>
            <div class="relative">
                <input class="form-input w-full px-sm py-[10px] rounded text-body-md text-on-background bg-surface-container-lowest pr-10" id="signup-password" name="password" placeholder="Create a strong password" type="password" required/>
            ...
            <div class="mt-2">
                <div class="flex justify-between items-center mb-1">
                    <span class="font-label-sm text-[10px] text-on-surface-variant">Password Strength</span>
                    <span class="font-label-sm text-[10px] text-on-surface-variant" id="strength-text">None</span>
                </div>
